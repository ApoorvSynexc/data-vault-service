...
- `getApexObjects({ user, mode, type })` — Lists accessible Salesforce objects. `mode` is what the objects are being listed for (`backup` | `archival` | `restore`), `type` the run style (`schedule` | `realtime`); both are normalised from untrusted query values by `toApexMode` / `toApexType`, which drop anything outside the contract. Returns the Apex reply as-is; Node applies no filtering of its own, so every call site (`crm-metadata.getsalesfroceObjects`, `backup-config.getObjectsHanlder`, `salesforce/metadata.syncMetadataAndTriggers`) sees exactly what Apex reports. Object eligibility is decided in Apex.
...
- `getApexObjectChilds({ user, objectName, mode, type, relationshipType, relationshipDepth })` — Child relationships. Same `mode`/`type` contract as `getApexObjects`, plus `relationshipType` (`MASTER` | `LOOKUP` | `REQUIRED_LOOKUP` | `ALL`) selecting which child relationships come back — this is what `type=MASTER` used to mean before `type` was repurposed. The archival object-tree endpoint requests `ALL`; the archival *job* path never calls this.
- `getMasterChildApiNames(user, objectName, type, mode = 'backup')` — Master-Detail child api names, via `relationshipType: 'MASTER'`.
- `getApexFields({ user, objectName, mode })` — Field metadata. `mode` only — schedule vs realtime does not change the fields.
...
- `createBackupJob(data)` — PutCommand with encrypted source + destination. Initial object statuses set. Runs `expandWithBackupChildren` first (re-enabled): for each selected object it fetches **all** children (`object-children?relationshipType=ALL`), writes the whole child payload to `schema/main/childs/{object}/childs.json` (+ this job's `delta/`), and adds only the backup-eligible ones — `isBackupChild`: Master-Detail, or `isRequired === true` — to the job, so each of those gets its own bulk query job. Optional lookups are stored but never queried. New children are appended to the backup config, so the tree deepens one level per run. Archival jobs never go through this.
- `getBackupJobById(id)` — GetItem.
- `updateBackupJob(id, updates)` — UpdateCommand.
