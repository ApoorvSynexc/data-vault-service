...
- `getApexObjects({ user, mode })` — Lists accessible Salesforce objects. Returns the Apex reply as-is; Node applies no filtering of its own, so every call site (`crm-metadata.getsalesfroceObjects`, `backup-config.getObjectsHanlder`, `salesforce/metadata.syncMetadataAndTriggers`) sees exactly what Apex reports. Object eligibility is decided in Apex.
