...
| GET | /restore/fetch-change-between-backup-jobs | restoreRetrieveJobController.fetchChangeBetweenBackupJobsHandler | Added 2026-07-30. `?backupConfigId=&startTime=&endTime=&limit=&cursor=` → the config's backupJobIds whose `startedAt` falls in the window, newest first. Feeds `source.backupJobIds` for a CHANGED_BETWEEN fetch-records call. ACL entry added to `middlewares/gateway/permissions/restore` (`restore.read`) — without it aclGateway 403s the path |
...
| POST | /restore/retrieve/show-preview | restoreRetrieveJobController.showPreviewHandler | Added 2026-07-30. Same body as fetch-records (`columns` optional and ignored) → `rows: [{ previous, current }]` — the restore-to version of each record paired with its live Salesforce record. ACL entry added to `middlewares/gateway/permissions/restore` (`restore.read`) — without it aclGateway 403s the path |
