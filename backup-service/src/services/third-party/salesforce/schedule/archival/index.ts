...
const FAILED_RECORDS_RETRY_STATUSES = new Set([OBJECT_STATUS.deletionRecordsFailed]);
...
const deleteNode = async (params: IDeleteNodeParams): Promise<string> => {
  ...
  if (failedSubtreeIds.has(node.id)) {
    logger.warn(
      `[archival:delete] skip — node already FAILED (cascaded from ancestor upload failure) | backupJobId:${backupJobId} objectName:${node.name}`
    );
  ...
  if (skipCompleted && node.status === OBJECT_STATUS.completed) {
    logger.info(
      `[archival:delete] skip (already completed) | backupJobId:${backupJobId} objectName:${node.name}`
    );
  ...
  if (hasBlockingChildFailure) {
    logger.warn(
      `[archival:delete] parent blocked — child DELETION_JOB_FAILED | backupJobId:${backupJobId} objectName:${node.name}`
    );
    await updateArchivalObject({
      backupJobId,
      object: {
        ...
        errorMessage: 'Blocked: one or more child objects failed deletion',
      },
    });
    return OBJECT_STATUS.deletionJobFailed;
  }
  ...
  if (!s3Keys.length) {
    if (uploadFailedIds.has(node.id)) {
      logger.warn(
        `[archival:delete] skip — upload failed, blocking parent | backupJobId:${backupJobId} objectName:${node.name}`
      );
      return OBJECT_STATUS.deletionJobFailed;
    }
    logger.info(
      `[archival:delete] skip (no S3 files / 0 records) | backupJobId:${backupJobId} objectName:${node.name}`
    );
    return OBJECT_STATUS.completed;
  }
  ...
  logger.info(
    `[archival:delete] deleting | backupJobId:${backupJobId} objectName:${node.name} s3FileCount:${s3Keys.length}`
  );
  const finalStatus = await bulkDeleteRecords({
    backupConfigId,
    backupJobId,
    crmId,
    crmName,
  ...
  logger.info(
    `[archival:delete] complete | backupJobId:${backupJobId} objectName:${node.name} finalStatus:${finalStatus}`
  );
  return finalStatus;
};
...
export const archiveAndHardDelete = async (
  ...
  logger.info(
    `[archival:orchestrator] started | backupConfigId:${backupConfigId} backupJobId:${backupJobId} objectId:${object.id} objectName:${objectName} incomingStatus:${object.status ?? 'none'} whereClause:"${whereClause || '(none)'}"`
  );
  ...
  const hasFailedDescendant = (node: IBackupObject): boolean =>
    node.status === OBJECT_STATUS.failed || (node.children ?? []).some(hasFailedDescendant);
  ...
  try {
    ...
    if (FAILED_RECORDS_RETRY_STATUSES.has(object.status ?? '')) {
      if (hasFailedDescendant(object)) {
        logger.info(
          `[archival:orchestrator] failed-records retry has FAILED descendants — falling through to full Phase 1+2+3 | backupJobId:${backupJobId} objectName:${objectName}`
        );
        // Fall through to Phase 1+2+3 below.
      } else {
        logger.info(
          `[archival:orchestrator] failed-records-only retry | backupJobId:${backupJobId} objectName:${objectName}`
        );
        await runFailedRecordsPhase({
          backupConfigId,
          backupJobId,
          crmId,
          crmName,
        ...
        logger.info(
          `[archival:orchestrator] failed-records-only retry complete | backupJobId:${backupJobId} objectName:${objectName}`
        );
        return;
      }
    }
    ...
    if (DELETE_ONLY_STATUSES.has(object.status ?? '')) {
      if (hasFailedDescendant(object)) {
        logger.info(
          `[archival:orchestrator] delete-only retry has FAILED descendants — falling through to full Phase 1+2+3 | backupJobId:${backupJobId} objectName:${objectName}`
        );
        // Fall through to Phase 1+2+3 below. Phase 1 resumes from existing bulkJobId.
      } else {
        logger.info(
          `[archival:orchestrator] delete-only retry — skipping phases 1 & 2 | backupJobId:${backupJobId} objectName:${objectName}`
        );

        const rebuildS3Urls = async (node: IBackupObject): Promise<void> => {
          if (
            DELETE_ONLY_STATUSES.has(node.status ?? '') ||
            node.status === OBJECT_STATUS.completed
          ) {
            const prefix = buildS3KeyPrefix({
              crmId,
              crmName,
              backupConfigId,
              backupJobId,
        ...
        await deleteNode({
          ...
          failedSubtreeIds: new Set(),
          node: object,
          skipCompleted: true,
        });
        logger.info(
          `[archival:orchestrator] delete-only retry complete | backupJobId:${backupJobId} objectName:${objectName}`
        );
        return;
      }
    }
    ...
    if (object.bulkJobId) {
      ...
      logger.info(
        `[archival:orchestrator] resuming existing bulk job | backupJobId:${backupJobId} objectName:${objectName} jobId:${jobId} totalRecordCount:${totalRecordCount}`
      );
    } else {
      await updateArchivalObject({
        backupJobId,
        object: { id: object.id, salesforceApiCount: 1, status: OBJECT_STATUS.bulkQueryInProgress },
      });
    ...
    logger.info(
      `[archival:orchestrator] phase 1 complete | backupJobId:${backupJobId} objectName:${objectName} totalRecordCount:${totalRecordCount}`
    );
    ...
    if (totalRecordCount === 0) {
      ...
      logger.info(
        `[archival:orchestrator] 0 records — marking root and subtree COMPLETED | backupJobId:${backupJobId} objectName:${objectName}`
      );
      await updateArchivalObject({
        backupJobId,
        object: {
    ...
    logger.info(
      `[archival:orchestrator] phase 2 starting — BFS upload | backupJobId:${backupJobId} objectName:${objectName}`
    );
    ...
    if (ROOT_UPLOAD_DONE_STATUSES.has(object.status ?? '')) {
      logger.info(
        `[archival:orchestrator] root upload already done — rebuilding S3 urls from listing | backupJobId:${backupJobId} objectName:${objectName} status:${object.status}`
      );
      const existingKeys = await listS3Objects(destConfig, archivePrefix);
      s3UrlsById.set(object.id, existingKeys);
    } else {
      const {
        s3UrlsPerObject: rootS3Map,
        totalSizeInBytes,
      ...
      for (let i = 0; i < currentLevel.length; i += CONCURRENCY_LIMIT) {
        ...
        const results = await Promise.allSettled(
          batch.map(async ({ object: node, parentWhereBody: pwb }) => {
            const { s3UrlsMap, effectiveWhereBody } = await uploadSingleObject(
              backupJobId,
              pwb,
              node,
              ctx
            ...
            if (
              !keys.length &&
              (node.status === OBJECT_STATUS.uploadCompleted ||
                node.status === OBJECT_STATUS.completed)
            ) {
              const prefix = buildS3KeyPrefix({
                crmId,
        ...
        for (let j = 0; j < results.length; j++) {
          ...
          if (result.status === 'fulfilled') {
            ...
            if (s3Keys.length > 0) {
              ...
              logger.info(
                `[archival:upload] 0 records — cascading COMPLETED to subtree | backupJobId:${backupJobId} objectName:${node.name}`
              );
            ...
            const errMsg =
              (result.reason as any)?.message ?? String(result.reason ?? 'parent upload failed');
            logger.warn(
              `[archival:upload] upload failed — cascading FAILED to subtree | backupJobId:${backupJobId} objectName:${failedNode.object.name} error:${errMsg}`
            );
            await cascadeFailureToSubtree(
              backupJobId,
              failedNode.object,
              failedSubtreeIds,
              `Parent ${failedNode.object.name} upload failed: ${errMsg}`
            );
          }
        }
      }
    ...
    // -------------------------------------------------------------------------
    logger.info(
      `[archival:orchestrator] phase 3 starting — post-order delete | backupJobId:${backupJobId} objectName:${objectName}`
    );
    await deleteNode({
      backupConfigId,
      backupJobId,
    ...
    logger.info(
      `[archival:orchestrator] completed | backupConfigId:${backupConfigId} backupJobId:${backupJobId} objectName:${objectName}`
    );
  } catch (err: any) {
    const errorMsg = err?.message ?? String(err);
    logger.error(
      `[archival:orchestrator] failed | backupConfigId:${backupConfigId} backupJobId:${backupJobId} objectName:${objectName} error:${errorMsg}`
    );
    await updateArchivalObject({
      backupJobId,
...
const runFailedRecordsPhase = async (params: {
  ...
  const { backupConfigId, backupJobId, crmId, crmName, instanceUrl, tokens, object, destConfig } =
    params;
  ...
  const processNode = async (node: IBackupObject) => {
    ...
    if (node.status === OBJECT_STATUS.deletionRecordsFailed) {
      logger.info(
        `[archival:orchestrator] failed-records retry | backupJobId:${backupJobId} objectName:${node.name} objectId:${node.id}`
      );
      const failedRecordsIdCsv = await buildFailedRecordsIdCsv(
        destConfig,
        crmId,
        crmName,
        backupConfigId,
      ...
      logger.info(
        `[archival:orchestrator] failed-records retry skip | backupJobId:${backupJobId} objectName:${node.name} status:${node.status}`
      );
    }
  };
